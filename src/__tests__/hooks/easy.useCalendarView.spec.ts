...
describe('엣지 케이스', () => {
  it('월간 뷰에서 2024-01-01 이전으로 navigate시 2023-12-01 날짜로 지정이 된다', () => {
    const { result } = renderHook(() => useCalendarView());
    act(() => {
      result.current.setCurrentDate(new Date('2024-01-01'));
    });
    ...
    assertDate(result.current.currentDate, new Date('2023-12-01'));
  });

  it('주간 뷰에서 2024-01-01 이전으로 navigate시 2023-12-25 날짜로 지정이 된다', () => {
    const { result } = renderHook(() => useCalendarView());
    act(() => {
      result.current.setCurrentDate(new Date('2024-01-01'));
    });
    ...
    assertDate(result.current.currentDate, new Date('2023-12-25'));
  });
});
